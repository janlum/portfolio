...
From the title of the challenge, we get a hint to check the headers of the website.
...
Looking at the source code, we find the first part of the flag: `picoCTF{t`.
...
We then get a clue to look in the apache access file, which a quick google search tells us that it's called the `.ht_access` file. Appending that to the url gives us the 4th part of the flag: `3s_2_lO0k`.

The next clue tells us to look for a file which stores information on a mac. Google then informs us that it's called a `.DS_Store` file. Looking in that file, we get the final part of the flag: `_a69684fd}`.

**Flag: picoCTF{t -→h4ts_4_l0t_0f_pl4c3s_2_lO0k_a69684fd}**
...
The website allows anonymous log in with no password, but we won't get a flag.

Using Burp to intercept the response packet, we can see that there is a cookie labelled 'admin'. Changing the value to 'True' brings us to the page with the flag. 
...
Looking at the source code, we se a javascript file which contains a function which splits the flag into 8 pieces. Piecing them together gives us the flag.
...
We are provided a URL to a webpage which contains two forms requesting for two PDFs. 
